...
const LOG_CHANNEL_ID = '1456197056988319869'; // Admin Log Channel
...
async function runGatekeeper(client) {
    ...
    const logToDiscord = async (title, content, colorHex) => {
        ...
        if (channel) {
            const c = new ContainerBuilder()
                ...
                .setAccentColor(colorHex); // 👈 Adds the color strip
            ...
            for (const [memberId, member] of satelliteGuild.members.cache) {
                ...
                if (isInMain) {
                    ...
                    if (!pendingKicks.has(kickKey)) {
                        ...
                        await logToDiscord(
                            '⏳ Timer Started', 
                            `**User:** ${member.user.tag}\n**Server:** ${satelliteGuild.name}\n**Status:** Not in main **A2-Q** server. **10** minutes countdown started.`,
                            0xFEE75C // Yellow
                        );
                        ...
                        if (timeDiff > TEN_MINUTES) {
                            try {
                                ...
                                await logToDiscord(
                                    '🥾 User Kicked', 
                                    `**User:** ${member.user.tag}\n**Server:** ${satelliteGuild.name}\n**Reason:** **10** minutes Timer Expired.`,
                                    0xED4245 // Red
                                );
