...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('info')
        .setDescription('Shows a dynamic help menu with categories'),
    ...
    async execute(interaction) {
        ...
        const response = await interaction.reply({
            ...
            fetchReply: true // Important for collectors!
        });

        // 4. Collector
        const collector = response.createMessageComponentCollector({ 
            componentType: ComponentType.StringSelect, 
            time: 60000 // changed from 'idle' to 'time' for standard timeout logic, feel free to swap back
        });
        ...
        collector.on('collect', async i => {
            ...
            if (selection === 'owner') await i.update({ embeds: [ownerEmbed] });
            else if (selection === 'admin') await i.update({ embeds: [adminEmbed] });
            else if (selection === 'moderation') await i.update({ embeds: [modEmbed] });
            else if (selection === 'registration') await i.update({ embeds: [registerEmbed] });
            else if (selection === 'general') await i.update({ embeds: [generalEmbed] });
