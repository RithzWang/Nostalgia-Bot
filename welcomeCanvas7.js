...
// Helper: Convert Integer Color (from API) to Hex
 ...
 * @param {Array<number>} [themeColors] - Optional: Array of [Primary, Accent] integers from unofficial API
 */
async function createWelcomeImage(member, themeColors = null) {
    ...
    if (themeColors && Array.isArray(themeColors) && themeColors.length === 2) {
        ...
        if (primaryHex && accentHex) {
            ...
            gradient.addColorStop(0, primaryHex); // Top
            gradient.addColorStop(1, accentHex);  // Bottom
            borderStyle = gradient;
        }
    } else if (user.hexAccentColor && isNitro) {
        // --- OPTION B: STANDARD SINGLE ACCENT (Fallback) ---
        const gradient = ctx.createLinearGradient(0, 0, 0, dim.height);
