...
function buildLogPayload(user, type, tagText, serverName, isMainServer, imageUrl) {
    ...
    // Dynamically format based on Main vs Local
    const userDisplay = isMainServer ? `<@${user.id}>` : `**${user.username}**`;
    
    let contentString = "";
    if (isMainServer) {
        // MAIN SERVER LOG: Drops "our", includes "from **Server**"
        contentString = isAdopt 
            ? `${userDisplay} starts adopting **${tagText}** tag from **${serverName}**`
            : `${userDisplay} stopped adopting **${tagText}** tag from **${serverName}**`;
    } else {
