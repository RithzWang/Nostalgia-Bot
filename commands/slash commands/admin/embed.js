    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('create').setDescription('Create a message embed')
                ...
                .addStringOption(option => option.setName('image').setDescription('Image Link (URL)')) // Added Image
        ...
        .addSubcommand(subcommand =>
            subcommand.setName('edit').setDescription('Edit an existing embed')
                ...
                .addStringOption(option => option.setName('image').setDescription('New Image Link (URL)')) // Added Image
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (subcommand === 'create') {
                ...
                const image = interaction.options.getString('image'); // Get Image
                const reuseMessageId = interaction.options.getString('message_id');

                if (!title && !description && !image) return interaction.reply({ content: '<:no:1297814819105144862> Provide Title, Description, or Image!', flags: MessageFlags.Ephemeral });

                const embed = new EmbedBuilder().setColor(color);
                if (title) embed.setTitle(title);
                if (description) embed.setDescription(description);
                if (image) embed.setImage(image); // Set Image
                ...
                if (reuseMessageId) {
                    ...
                    // If reusing a message, we often want to clear the original content if it exists
                    ...
                    // Removed Publish/Crosspost logic here
                ...
                if (newImage) newEmbed.setImage(newImage); // Set New Image
